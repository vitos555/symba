...
template<class FieldClass, class AllocatorsClass=Allocators<FieldClass> > class Monomial {
    ...
    public:
        ...
        // Should be doing following
        // 16x^2z & 6xy = 2 x
        MonomialClass intersect(const MonomialClass &other, const side_type &side=side_type::both) {
            ...
            return MonomialClass(intersection);
        }
};
        ...
        void factor(const side_type &side=side_type::both) {
            if (size() > 1) {
                ...
                while(cur_term != terms.end()) {
                    intersection = cur_term->intersect(intersection, side);
